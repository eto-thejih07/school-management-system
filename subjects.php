...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_selected'])) {
    if (!empty($_POST['selected_subjects'])) {
        ...
        $delete_result = deleteMultipleSubjects($subject_ids);
        
        if ($delete_result) {
            header("Location: subjects.php?success=" . urlencode(count($subject_ids) . " subjects deleted successfully"));
            exit();
        } else {
            $error = "Failed to delete selected subjects. Please try again.";
        }
    } else {
        $error = "No subjects selected for deletion.";
    }
}

// Handle single delete via GET
if (isset($_GET['delete_id'])) {
    $delete_result = deleteSubject($_GET['delete_id']);
...
<body>
        ...
        <div class="main">
            ...
            <div class="sectionHeader">
                ...
                <div class="headerActions">
                    <a href="subject_register.php" class="btn btn-primary">
                        ...
                        <table class="data-table">
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    ...
                                    <tr>
                                        <td>
                                            <input type="checkbox" name="selected_subjects[]" value="<?php echo htmlspecialchars($subject['subject_id']); ?>" class="subject-checkbox">
                                        </td>
                                        ...
                                        <td>
                                            <span class="badge"><?php echo htmlspecialchars($subject['teacher_count'] ?? 0); ?> teachers</span>
                                        </td>
                                        <td>
                                            <?php 
                                        ...
                                        <td>
                                            <span class="status <?php echo ($subject['is_active'] == 1) ? 'active' : 'inactive'; ?>">
                                                <?php echo ($subject['is_active'] == 1) ? 'Active' : 'Inactive'; ?>
                                            </span>
                                        </td>
                                        <td>
                                            <div class="actions">
                                                <a href="subject_view.php?id=<?php echo $subject['subject_id']; ?>" class="btn-action view" title="View Subject">
                                                    <i class="fas fa-eye"></i>
                                                    <span class="btn-text">View</span>
                                                </a>
                                                <a href="subject_edit.php?id=<?php echo $subject['subject_id']; ?>" class="btn-action edit" title="Edit Subject">
                                                    <i class="fas fa-edit"></i>
                                                    <span class="btn-text">Edit</span>
                                                </a>
                                                <button type="button" class="btn-action delete" 
                                                        onclick="confirmDelete('<?php echo htmlspecialchars($subject['subject_id']); ?>', '<?php echo htmlspecialchars(addslashes($subject['subject_name'])); ?>')"
    ...
    <script>
        ...
        function confirmBulkDelete() {
            const checkedBoxes = document.querySelectorAll('.subject-checkbox:checked');
            if (checkedBoxes.length > 0) {
                if (confirm(`Are you sure you want to delete ${checkedBoxes.length} selected subject(s)? This will remove these subjects from all teachers.`)) {
        ...
        function confirmDelete(subjectId, subjectName) {
            if (confirm(`Are you sure you want to delete subject "${subjectName}" (ID: ${subjectId})? This will remove this subject from all teachers.`)) {
                window.location.href = `subjects.php?delete_id=${encodeURIComponent(subjectId)}`;
            }
        }
    </script>
